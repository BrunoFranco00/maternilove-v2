...
4. Deve navegar para `/app/relief` com resposta imediata (título, mensagem, sugestão)
...
### 4. Persistência local (modo demo)
...
| Arquivo | Alteração |
|---------|-----------|
| `frontend/src/middleware.ts` | Bypass de auth quando `NEXT_PUBLIC_AUTH_DISABLED=true` |
| `frontend/src/app/app/(core-emotional)/core.store.ts` | Check-in local quando auth desabilitado |
| `frontend/src/app/app/(core-emotional)/check-in/page.tsx` | UI premium, sem redirect para login quando auth desabilitado |
...
| `frontend/.env.example` | **Novo** — documenta NEXT_PUBLIC_AUTH_DISABLED |
